...
# Creates dbt Platform env vars so each environment can point to different source schemas.
# In dbt_project.yml: source_database: "{{ env_var('DBT_SOURCE_DATABASE', 'ANALYTICS') }}"
# Only created when source_database / source_schema_prefix are non-empty in tfvars.
